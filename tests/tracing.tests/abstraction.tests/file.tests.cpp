...
TEST_F(file, mmap)
{
	...
	EXPECT_TRUE(temp_file);
	// ASAN's shadow memory layout may make the computed address writable, and
	// TSAN does not fault on out-of-bounds access, so skip the death test there
