...
class TestLiveExtremeStress(unittest.TestCase):
    """
    ...
    def test_high_volume_tracepoints_with_summary(self):
        ...
        Writes many tracepoints rapidly and verifies that the summary
        correctly reports read, output, and dropped counts.
        """
    ...
    def test_buffer_overflow_with_small_buffer(self):
        ...
        Uses a very small buffer size to force dropping of tracepoints,
        and verifies that the decoder doesn't crash and reports drops correctly.
        """
    ...
    def test_multiple_buffers_high_volume(self):
        ...
        Creates several buffers and writes many tracepoints to each,
        verifying that the decoder can handle concurrent high-volume
    ...
    def test_sustained_high_frequency_writes(self):
        ...
        Runs decoder while continuously writing tracepoints at high frequency
        to simulate a heavily-traced application.
        """
        import threading
        ...
        try:
            ...
            # Give decoder time to process remaining tracepoints
            time.sleep(1)
            ...
            # Verify we read all written messages
    ...
    def test_unlimited_buffer_no_drops(self):
        ...
        Writes many tracepoints with unlimited buffer and verifies
        that no drops occur.
        """
        ...
        # Run with unlimited buffer (size=0)
        result = subprocess.run(
            ["timeout", "3", str(clltk_path), "live", self.trace_path,
    ...
    def test_interleaved_multi_buffer_ordering(self):
        ...
        Writes interleaved tracepoints to multiple buffers with controlled
        timing and verifies output is in timestamp order.
        """
        clltk_path = get_clltk_path()
        num_buffers = 3
        rounds = 20
        ...
        # Write interleaved messages with small delays to ensure distinct timestamps
        messages_in_order = []
        ...
        result = subprocess.run(
            ["timeout", "3", str(clltk_path), "live", self.trace_path,
             "--order-delay", "30", "--poll-interval", "2"],
            capture_output=True, env=os.environ.copy()
        )
        ...
        # Verify ordering - find positions of each message
        positions = []
        for msg in messages_in_order:
            pos = stdout_text.find(msg)
            if pos >= 0:
    ...
    def test_rapid_start_stop_cycles(self):
        ...
        Starts and stops the decoder multiple times rapidly to verify
        there are no resource leaks or race conditions.
        """
        ...
        # Write some initial tracepoints
        for i in range(20):
            subprocess.run(
        ...
        for cycle in range(num_cycles):
            # Write a unique message for this cycle
            ...
            # Decoder should not crash (timeout returns 124)
    ...
    def test_maximum_message_throughput(self):
        ...
        This is a benchmark test that writes as many messages as possible
        and measures how many the decoder can process without dropping.
        """
        import threading
    ...
    def test_stress_with_varying_message_sizes(self):
        ...
        Writes messages of different sizes rapidly to test decoder
        handling of variable-length data under load.
        """
