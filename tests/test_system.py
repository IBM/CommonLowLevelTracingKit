    ...
    def test_random_constructor(self: unittest.TestCase):
        for language in [Language.C, Language.CPP]:
            with self.subTest(language=language):
                ...
                self.assertEqual(
                    tracepoints["formatted"][TRACEBUFFER_INFO_COUNT],
                    "should be in tracebuffer",
                )
                pass
    ...
    def test_misuse_of_tracebuffer(self: unittest.TestCase):
        for language in [Language.C, Language.CPP]:
            with self.subTest(language=language):
                ...
                stderr: str = process(
                    file_content, language=language, run_musst_fail=True
                )
                self.assertRegex(
                    stderr, r"^\x1b\[1;31mclltk recoverable: could not open tracebuffer"
                )
                pass
