...
---
=== span events
Span begin/end reuse the complete static tracepoint machinery: a span-begin is
a meta entry of type 3 whose string is the span name and whose arguments are
two ordinary uint64 values (span id, parent id or 0); a span-end is type 4
with one uint64 (span id). The trace entry layout is unchanged, so decoders
read span arguments with the existing argument machinery and only the
rendering and correlation differ by meta type. Span ids come from
`clltk_next_span_id()`: a per-process random 32 bit salt combined with a
...
=== unique stack lookup index
Registration lookups (deduplicating meta entries) go through an in-memory
open-addressing table per tracebuffer: content hash to entry offset, with
candidates verified against the file (md5 in the entry head, or byte
comparison on the batch path). The table loads from the last persisted slab
in the stack plus a tail scan and is republished as a tagged, nibble-encoded
stack entry when enough new entries accumulated. The append-only stack stays
the source of truth; the slab is discardable cache.
