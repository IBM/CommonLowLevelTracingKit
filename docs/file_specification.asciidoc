...
==== Byte order
All multi-byte fields are stored in the native byte order of the writing
machine. The byte order is identified by the magic: the writer stores the
magic as two native-endian 64 bit words, so a little-endian file starts with
the bytes `?#$~tracebuffer\0` and a big-endian file (e.g. written on s390x)
starts with `cart~$#?\0reffube`. Decoders detect the byte order from the
magic and byte-swap all multi-byte fields when reading a file written with
...
While the ringbuffer holds the traceentries, the stack hold the corresponding meta data necessary to decode the traceentries.

Since 1.7.0 the 8 reserved bytes in every stack entry head tag the entry
kind: all zero = meta blob (the only kind before 1.7.0), `CLLTKIDX` = a
persisted lookup index slab written by the tracing library to avoid linear
stack scans. Slab bodies are nibble-encoded - every byte has the high bit
set - so decoders older than 1.7.0 that scan entry bodies for the meta magic
`{` (0x7B) can never match inside a slab and skip it safely. Slabs are pure
caches: the last valid slab wins, and a missing or unreadable slab only
causes the writer to rebuild its index by scanning the stack. A crash while
writing a slab leaves it beyond the committed stack body size and therefore
invisible. Multiple meta entry are combined into meta blobs which are than stored, each meta blob as one stack entry, inside the stack. 
...
The meta_entry_type differentiates the tracepoint styles:

* 1 = printf-like tracepoint (format string with printf specifiers)
* 2 = dump tracepoint (message string, one flex-size binary argument)
