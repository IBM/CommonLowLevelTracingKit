...
### ELF Sections
- The unique stack (in the trace file) contains tagged entries since 1.7.0: zero tag = meta blob, `CLLTKIDX` = persisted lookup index slab (nibble-encoded, skippable by old decoders, rebuildable by scan). Meta lookups go through the in-memory index; never reintroduce linear stack scans on hot paths.
