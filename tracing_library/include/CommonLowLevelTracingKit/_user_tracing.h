...
__attribute__((constructor(101), used)) static void _clltk_constructor(void)
{
	...
	for (_clltk_tracebuffer_handler_t *const *handler_ptr = &__start__clltk_tracebuffer_handler_ptr;
		...
		if ((handler->meta.file_offset == _clltk_file_offset_unset) && (meta_size > 0)) {
			handler->meta.file_offset = _clltk_tracebuffer_add_to_stack(
				handler->tracebuffer, handler->meta.start, meta_size);
		}
	}
}
...
#define _CLLTK_STATIC_TRACEBUFFER(_NAME_, _SIZE_)                                            \
	...
	static _clltk_tracebuffer_handler_t _clltk_##_NAME_                                      \
		__attribute__((used)) = {{#_NAME_, _SIZE_},                                          \
								 ...
								 NULL};                                                      \
                                                                                             \
	static _clltk_tracebuffer_handler_t *const _clltk_##_NAME_##_handler_ptr                 \
...
#define _CLLTK_STATIC_TRACEPOINT(_BUFFER_, _FORMAT_, ...)                                       \
	do {                                                                                        \
		...
		static _clltk_argument_types_t _clltk_types = _CLLTK_CREATE_TYPES(__VA_ARGS__);         \
                                                                                                \
		static _clltk_tracebuffer_handler_t *const hdl = &_clltk_##_BUFFER_;                    \
                                                                                                \
		/* ------- runtime time stuff ------- */                                                \
		static _clltk_file_offset_t _clltk_offset = _clltk_file_offset_unset;                   \
		if (_clltk_offset == _clltk_file_offset_unset) {                                        \
			_clltk_offset = _clltk_tracebuffer_get_in_file_offset(hdl, &_meta, sizeof(_meta));  \
		}                                                                                       \
                                                                                                \
		/* at runtime execute trace point */                                                    \
		_clltk_static_tracepoint_with_args(hdl->tracebuffer, _clltk_offset, __FILE__, __LINE__, \
	...
	do {                                                                                       \
		...
		if (_clltk_offset == _clltk_file_offset_unset) {                                       \
			_clltk_offset = _clltk_tracebuffer_get_in_file_offset(hdl, &_meta, sizeof(_meta)); \
                                                                                               ...
                                                                                               \
		/* at runtime execute trace point */                                                   \
		_clltk_static_tracepoint_with_dump(hdl->tracebuffer, _clltk_offset, _meta.file,        \
										   _meta.line, _ADDRESS_, _SIZE_);                     \
	} while (0)
