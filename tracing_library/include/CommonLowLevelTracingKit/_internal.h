...
void _clltk_static_tracepoint_with_args(_clltk_tracebuffer_t *buffer,
										...
										...)
	__attribute__((nonnull(3, 5), format(printf, 6, 7), used, visibility("default")));

static inline _clltk_file_offset_t
_clltk_tracebuffer_get_in_file_offset(_clltk_tracebuffer_handler_t *const handler,
									  const void *const this_meta, const uint32_t this_meta_size)
{
	const uintptr_t elf_sec_start = (uintptr_t)handler->meta.start;
	const uintptr_t elf_sec_stop = (uintptr_t)handler->meta.stop;
	const uintptr_t this_start = (uintptr_t)this_meta;
	const uintptr_t this_stop = (uintptr_t)this_meta + (uintptr_t)this_meta_size;

	if (!_CLLTK_FILE_OFFSET_IS_STATIC(handler->meta.file_offset))
	...
	if ((elf_sec_start <= this_start) && (this_stop <= elf_sec_stop)) {
		...
		return handler->meta.file_offset + (_clltk_file_offset_t)(this_start - elf_sec_start);
	} else {
		// if meta for this tracepoint is not in section
		// this will happen with template functions
		return _clltk_tracebuffer_add_to_stack(handler->tracebuffer, this_meta, this_meta_size);
	}
}
