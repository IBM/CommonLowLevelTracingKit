...
#define CLEANUP(_X_) __attribute__((unused, cleanup(_X_)))
...
#define SYNC_GLOBAL_LOCK(_NAME_) \
	syn_lock_object_t _NAME_ CLEANUP(syn_global_lock_release) = sync_global_lock_get()
...
#define SYNC_MEMORY_LOCK(_NAME_, _PTR_) \
	syn_lock_object_t _NAME_ CLEANUP(sync_memory_mutex_release) = sync_memory_mutex_get(_PTR_)
